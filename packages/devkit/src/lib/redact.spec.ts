...
const order = {
  customer: { name: 'Jane Doe', email: 'jane@mail.test', vip: true, discount: null },
...
describe('redactString', () => {
  it('keeps length and character classes', () => {
    expect(redactString('jane@mail.test')).toBe('aaaa@aaaa.aaaa');
...
describe('redactData', () => {
  it('redacts every string and keeps numbers, booleans, null and the shape', () => {
    expect(redactData(order)).toEqual({
      customer: { name: 'Aaaa Aaa', email: 'aaaa@aaaa.aaaa', vip: true, discount: null },
  ...
  it('redacts only the selected paths', () => {
    ...
    expect(result.customer).toEqual({ name: 'Aaaa Aaa', email: 'aaaa@aaaa.aaaa', vip: true, discount: null });
  ...
  it('redacts a whole branch, ignores paths that match nothing and never mutates the input', () => {
    ...
    expect(result.customer.email).toBe('aaaa@aaaa.aaaa');
